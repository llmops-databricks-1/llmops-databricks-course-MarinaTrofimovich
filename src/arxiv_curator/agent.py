...
FINAL_POST_INSTRUCTION = (
    "Return the final answer as a single polished LinkedIn post. "
    "Start with a strong hook, synthesize 2-3 relevant papers into one "
    "narrative, mention paper titles and arXiv IDs inline, and end with "
    "2-3 hashtags on the final line. "
    "Do not return bullet lists, numbered lists, paper inventories, or preambles such as "
...
class ArxivAgent(ResponsesAgent):
    ...
    def execute_tool(self, tool_name: str, args: dict[str, object]) -> object:
    ...
    def _prepare_request_input(
        ...
        """Augment the latest user message with strict formatting instructions."""
        prepared_input = [dict(item) for item in request_input]
        for item in reversed(prepared_input):
            if item.get("role") != "user":
                continue
            content = item.get("content")
            if isinstance(content, str) and FINAL_POST_INSTRUCTION not in content:
    ...
    def call_and_run_tools(
        ...
        prepared_request_input = self._prepare_request_input(request_input)
        messages: list[dict[str, Any]] = [
            {"role": "system", "content": self.system_prompt},
        ]
        if previous_messages:
            messages.extend(previous_messages)
        messages.extend(prepared_request_input)
        ...
        if session_id and self.memory:
            self.save_memory(
                session_id,
                prepared_request_input + self._extract_output_items(events),
            )
        return events
