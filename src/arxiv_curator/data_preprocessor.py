...
class DataProcessor:
    ...
    def _prepare_storage(self) -> None:
        """Ensure the catalog schema, volume, and run directory exist."""
        ...
        self.dbutils.fs.mkdirs(self.pdf_dir)
        logger.info(f"Prepared volume path: {self.pdf_dir}")
    ...
    def _get_range_start(self) -> str:
        ...
        if self.spark.catalog.tableExists(self.papers_table):
            ...
            if max_processed is not None:
                ...
                logger.info(
                    "Found existing arxiv_papers table with null watermark. "
    ...
    def download_and_store_papers(
        ...
        client = arxiv.Client()
        search = arxiv.Search(
            query=(f"(cat:cs.AI OR cat:cs.LG) AND submittedDate:[{start} TO {self.end}]"),
            max_results=100,
