...
linkedin_post_format_guideline = Guidelines(
    name="linkedin_post_format",
    guidelines=[
        "The response must be a single LinkedIn-style post, not a list of papers",
        (
            "The response must synthesize the research into a cohesive "
            "narrative rather than enumerate results"
        ),
        (
            "The response must not start with phrases like 'Here are a few "
            "papers' or similar list-style preambles"
        ),
        "The response should end with 2-3 relevant hashtags",
    ],
    model="databricks:/databricks-gpt-oss-120b",
)
...
def evaluate_agent(
    ...
    return mlflow.genai.evaluate(
        ...
        scorers=[
            ...
            linkedin_post_format_guideline,
        ],
    )
